...
Irradiance = 134.6  # kW/cm^2
scan_speed = 4.0    # nm/ms
...
[Materials]
    ...
    [constants] # Material Properties Constants
        ...
        prop_values = '0.4      10.0e-9     1.5     2.0e-10     2.0e-10     2.0e-6      2.0e-10     8.31' #sigma -> J/m^2; delta -> meter; M_si unit-> m^5/Js
    []
    ...
    [Mobility_HCP]    # Temperature dependent Mobility of Phase HCP              # http://arfc.github.io/software/moltres/wiki/input_example/  # exponential expression for temp dep.
        ...
        constant_names = 'fa_1 F_M1 F_temp M_1 Q_1'
        constant_expressions = '0 1.0 1.0 1.29e-06 6378'
        coupled_variables = 'temp'
    ...
    [Mobility_IMC]    # Temperature dependent Mobility of Phase IMC
        ...
        property_name = M2
        material_property_names = 'R'
        constant_names = 'fa_2 F_M2 F_temp M_2 Q_2'
    ...
    [Mobility_LIQUID]    # Temperature dependent Mobility of Phase LIQUID
        ...
        property_name = M3
        material_property_names = 'R'
        constant_names = 'fa_3 F_M3 F_temp M_3 Q_3'
        constant_expressions = '0 1.0 1.0 3.39e-06 3043'
        coupled_variables = 'temp'
    ...
    [Mobility_FCC]    # Temperature dependent Mobility of Phase FCC
        ...
        property_name = M4
        material_property_names = 'R'
        constant_names = 'fa_4 F_M4 F_temp M_4 Q_4'
        constant_expressions = '0 1.0 1.0 2.03e-06 7501'
        coupled_variables = 'temp'
    ...
    [L1-2] # Interfacial mobility between Phase HCP-IMC
        type = ParsedMaterial
        property_name = L1_2 
        ...
        expression = 'factor_L*(0.5/0.2)*(M1+M2)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))'  ## 0.2 ==> total values of dinominator of Eq 21 (https://doi.org/10.1016/j.ijmecsci.2020.105843)
    []

    [L2-3] # Interfacial mobility between Phase IMC-IMC
        type = ParsedMaterial
        property_name = L2_3
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M2 M_gb energy_scale time_scale length_scale mu kappa h2 h3 h4 h5'
        expression = 'factor_L*((0.5/0.2)*(M2+M2)+M_gb*(h2*h3+h3*h4+h4*h5))*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []

    [L2-7] # Interfacial mobility between Phase IMC-FCC
        type = ParsedMaterial
        property_name = L2_7
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M2 M4 energy_scale time_scale length_scale mu kappa'
        expression = 'factor_L*(0.5/0.2)*(M2+M4)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []

    [L2-6]  # Interfacial mobility between Phase IMC-LILQUID
        type = ParsedMaterial
        property_name = L2_6
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M2 M3 energy_scale time_scale length_scale mu kappa'
        expression = 'factor_L*(0.5/0.2)*(M2+M3)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []

    [L1-7] # Interfacial mobility between Phase HCP-FCC
        type = ParsedMaterial
        property_name = L1_7
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M1 M4 energy_scale time_scale length_scale mu kappa'
        expression = 'factor_L*(0.5/0.2)*(M1+M4)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []

    [L1-6] # Interfacial mobility between Phase HCP-LIQUID
        type = ParsedMaterial
        property_name = L1_6
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M1 M3 energy_scale time_scale length_scale mu kappa'
        expression = 'factor_L*(0.5/0.2)*(M1+M3)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []

    [L6-7]  # Interfacial mobility between Phase LIQUID-FCC
        type = ParsedMaterial
        property_name = L6_7
        constant_names = 'factor_L'
        constant_expressions = '1.0'
        material_property_names = 'M3 M4 energy_scale time_scale length_scale mu kappa'
        expression = 'factor_L*(0.5/0.2)*(M3+M4)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []
    ...
    [Interface_Mobility]
        ...
        material_property_names = 'L1_2 L2_3 L2_7 L1_7 L6_7 L1_6 L2_6 h1 h2 h3 h4 h5 h6 h7' 
